...
void relax()
{
	...
	//sending matrix slice to upper neighbor
	if (count != 1) {
		for (i = 0; i < N; ++i) {
			if (rank % 2 == 0) {
				if (rank != 0) {
	...
	for (blockNum = 0; blockNum < count; ++blockNum) {
		//receiving block slice from upper neighbor
		if (rank != 0) {
		...
		for (i = 0; i < blockSize; ++i) {
			for (j = 0; j < blockSize; ++j) {
				for (k = 0; k < N; ++k) {
					...
					if (rank == 1 && blockNum == 0 && i == 0 && j == 0 && k == 0) {
		...
		//sending block slice to down neighbor
		if (rank != count - 1) {
			for (i = 0; i < blockSize; ++i) {
